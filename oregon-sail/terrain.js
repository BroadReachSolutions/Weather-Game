  ...
  async function classifyTile(tileX, tileY, zoom, gridSize, googleSession, googleKey) {
    if (googleSession && googleKey) {
      try {
        ...
        console.warn("Oregon Sail terrain: Google terrain tile failed", e);
      }
    }
    /* No fallback -- return all-land rather than silently classifying
       with a different source that uses different colors and would
       produce wrong results with the Google-tuned classifier */
    const emptyGrid = Array.from({ length: gridSize }, () => Array(gridSize).fill("land"));
    return { grid: emptyGrid, source: "failed" };
  }
